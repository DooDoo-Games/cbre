...
namespace CBRE.Providers {
    ...
    public class GenericStructure {
        ...
        public Tuple<Vector3, decimal, decimal> PropertyTextureAxis(string name) {
            var prop = this[name];
        ...
        private static object ConvertPrimitive(string primitiveType, string value) {
            ...
            switch (primitiveType) {
                ...
                case "Vector3":
                    return Vector3.Parse(spl[0].TrimStart('('), spl[1], spl[2].TrimEnd(')'));
                case "Box":
                    return new Box(
                        Vector3.Parse(spl[0].TrimStart('('), spl[1], spl[2].TrimEnd(')')),
